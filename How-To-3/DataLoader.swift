...
/*
    ...
    - That is to say, that a URLSessionDataTask is an object/class that takes care of doing network call asynchronously. If you check the method for dataTask, all it does is return a URLSessionDataTask object.
    - The closure is basically handling what that newly create object returns.
 */

extension URLSession: DataLoader {
    // When nesting completion handlers, the thing to keep in mind is that we're sending data back to wherever the top layered method is called.
    // Always remember to call completion with only the data you want sent back.
    func loadData(from request: URLRequest, completion: @escaping (Data?, Error?) -> Void) {
        // Create a DataTask with the request: An URLRequest can be built up to contain headers and encoded data in the body.
        // We use this when we want to send data to the server
        dataTask(with: request) { data, _, error in
            // We can choose to handle the usual checking for data or error in here. But we can also worry about handling that within whichever controller uses these methods. This gives rooom for custom error handling and data handling if the app was much larger in scale.
